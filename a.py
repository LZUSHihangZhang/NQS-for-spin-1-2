...
def Neel_state(Lx, Ly):
    state = torch.ones((Lx, Ly,3), device=device, dtype=dtype1)
    for i in range(Lx):
        for j in range(Ly):
            if (i + j) % 2 == 0:
                state[i, j,:] = 1
...
class Kagome:
    ...
    def mapping(self, states):
        mapping_state = torch.zeros((states.shape[0],self.lx, self.ly,6),dtype=dtype1,device=device)
        mapping_state[:,:,:,0] = torch.roll(states,dims=2,shifts=-1)[:,:,:,0]
        mapping_state[:,:,:,1] = torch.roll(states,dims=2,shifts=-1)[:,:,:,2]
        mapping_state[:,:,:,2] = states[:,:,:,1]
        mapping_state[:,:,:,3] = states[:,:,:,2]
        mapping_state[:,:,:,4] = torch.roll(states,dims=1,shifts=-1)[:,:,:,0]
        mapping_state[:,:,:,5] = torch.roll(states,dims=1,shifts=-1)[:,:,:,1]
    ...
    def select_sample(self, state):
        with torch.no_grad():
            ...
            '''产生子格的随机数'''
            R1 = torch.randint(0, 3, (self.step,), device=device)
            R2 = torch.randint(0, 3, (self.step,), device=device)
            '''为了满足U(1)对称性我们要选择两个相反自旋的格点'''
            spin1 = state[torch.arange(self.step, device=device), Rx_1, Ry_1,R1]
            spin2 = state[torch.arange(self.step, device=device), Rx_2, Ry_2,R2]
            # 获取需要更新的样本索引
            ...
            R1_updates = R1[update_indices]
            R2_updates = R2[update_indices]

            # 创建新状态并翻转
            new_states_updates = states_to_update.clone()
            batch_idx = torch.arange(num_updates, device=device)
            new_states_updates[batch_idx, Rx_1_updates, Ry_1_updates,R1_updates] = -new_states_updates[
                batch_idx, Rx_1_updates, Ry_1_updates,R1_updates]
            new_states_updates[batch_idx, Rx_2_updates, Ry_2_updates,R2_updates] = -new_states_updates[
                batch_idx, Rx_2_updates, Ry_2_updates,R2_updates]
            # 计算新状态的概率
    ...
    def E_loc(self, states, ln_psi):
        with torch.no_grad():
            ...
            """子格内相互作用"""
            E += torch.sum(states[:, :, :, 0] * states[:, :, :, 1], dim=(1, 2)) / 4
            diff = torch.stack(torch.where(states[:, :, :, 0] != states[:, :, :, 1]))
            states_new = states[diff[0], :, :, :]
            rows = torch.arange(states_new.shape[0])
            states_new[rows, diff[1], diff[2], 0] = -states_new[rows, diff[1], diff[2], 0]
            ...
            E += torch.sum(states[:, :, :, 1] * states[:, :, :, 2], dim=(1, 2)) / 4
            diff = torch.stack(torch.where(states[:, :, :, 1] != states[:, :, :, 2]))
            states_new = states[diff[0], :, :, :]
            rows = torch.arange(states_new.shape[0])
            states_new[rows, diff[1], diff[2], 1] = -states_new[rows, diff[1], diff[2], 1]
            states_new[rows, diff[1], diff[2], 2] = -states_new[rows, diff[1], diff[2], 2]
            ln_psi_new = self.ViT(states_new)
            values = torch.exp(ln_psi_new - ln_psi[diff[0]]) * 0.5
            E.index_add_(0, diff[0], values)

            E += torch.sum(states[:, :, :, 2] * states[:, :, :, 0], dim=(1, 2)) / 4
            diff = torch.stack(torch.where(states[:, :, :, 2] != states[:, :, :, 0]))
            states_new = states[diff[0], :, :, :]
            rows = torch.arange(states_new.shape[0])
            states_new[rows, diff[1], diff[2], 2] = -states_new[rows, diff[1], diff[2], 2]
            states_new[rows, diff[1], diff[2], 0] = -states_new[rows, diff[1], diff[2], 0]
            ...
            E += torch.sum(states[:,:,:,0] * states_new[:,:,:,2], dim=(1, 2))/ 4
            diff = torch.stack(torch.where(states[:,:,:,0] != states_new[:,:,:,2]))
            states_new = states[diff[0], :, :]
            rows = torch.arange(states_new.shape[0])
            states_new[rows, diff[1], diff[2],0] = -states_new[rows, diff[1], diff[2],0]
            states_new[rows, diff[1] - 1, diff[2],2] = - states_new[rows, diff[1] - 1, diff[2],2]
            ln_psi_new = self.ViT(states_new)
            values = torch.exp(ln_psi_new - ln_psi[diff[0]]) * 0.5
            E.index_add_(0, diff[0], values)

            states_new = torch.roll(states, shifts=1, dims=2)
            E += torch.sum(states[:,:,:,0] * states_new[:,:,:,1], dim=(1, 2))/4
            diff = torch.stack(torch.where(states[:,:,:,0] != states_new[:,:,:,1]))
            states_new = states[diff[0], :, :]
            rows = torch.arange(states_new.shape[0])
            states_new[rows, diff[1], diff[2],0] = -states_new[rows, diff[1], diff[2],0]
            states_new[rows, diff[1], diff[2]-1,1] = - states_new[rows, diff[1], diff[2] - 1,1]
            ln_psi_new = self.ViT(states_new)
            values = torch.exp(ln_psi_new - ln_psi[diff[0]]) * 0.5
            E.index_add_(0, diff[0], values)

            states_new = torch.roll(states, shifts=(1, -1), dims=(1, 2))
            E += torch.sum(states[:,:,:,1] * states_new[:,:,:,2], dim=(1, 2)) / 4
            diff = torch.stack(torch.where(states[:,:,:,1] != states_new[:,:,:,2]))
            states_new = states[diff[0], :, :]
            rows = torch.arange(states_new.shape[0])
            states_new[rows, diff[1], diff[2],1] = -states_new[rows, diff[1], diff[2],1]
            states_new[rows, diff[1]-1, (diff[2]+1)%self.ly,2] = - states_new[rows, diff[1]-1,(diff[2]+1)%self.ly,2]
            ln_psi_new = self.ViT(states_new)
            values = torch.exp(ln_psi_new - ln_psi[diff[0]]) * 0.5
            E.index_add_(0, diff[0], values)
    ...
    def learning(self):
        ...
        '''我们复制Neel态,得到一批Neel态'''
        states = states.repeat(self.step, 1, 1, 1)
        ...
        for i in range(10000):
            ...
            print('*************************************************')
            print('第', i, '步')
            print('能量为', E_mean.item() / self.lx / self.ly/3)
            E = -self.tau * (E - E_mean)
