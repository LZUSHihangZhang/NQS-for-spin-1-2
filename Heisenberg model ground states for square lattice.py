...
class Square:
    def __init__(self, lx, ly):
        ...
        self.W0 = torch.randn(self.nl, self.d, self.d, device=device, dtype=dtype1)
        ...
        self.eta = torch.ones(1, device=device, dtype=dtype1)
        ...
        self.parameter_size =(self.V.numel() + self.emb_matrix.numel()
                                   ...
                                   + self.gamma.numel() + self.eta.numel()) + 2*(self.omega_im.numel() + self.B_im.numel())
        self.step = 500
        self.warm_up = 100
        '''设置虚时间步'''
        self.tau = 0.05
    ...
    def mapping(self, states):
        """states[batch_size,lx,ly]->mapping[batch_size,lx,ly,9]"""
        mapping = torch.zeros(states.shape[0], self.lx, self.ly, 5, device=device, dtype=dtype1)

        mapping[:, :, :, 0] = torch.roll(states, dims=1, shifts=1)
        mapping[:, :, :, 1] = torch.roll(states, dims=2, shifts=1)
        mapping[:, :, :, 2] = states
        mapping[:, :, :, 3] = torch.roll(states, dims=2, shifts=-1)
        mapping[:, :, :, 4] = torch.roll(states, dims=1, shifts=-1)
        return mapping

    def ViT(self, states):
        batch_size = states.shape[0]
        X = self.mapping(states)[:,:,:,D4].permute(0,3,1,2,4).reshape(batch_size*self.groups,self.N,self.b)
        ...
        for i in range(self.nl):
            ...
            '''self.W0[d,d]'''
            y =( self.ReLU(torch.tensordot(A, self.W0[i, :, :], dims=([2], [1]))) + A)
    ...
    def learning(self):
        ...
        for i in range(10000):
            ...
            params_list = [
                ...
                self.gamma, self.beta, self.concate, self.eta
            ]
            ...
            for u in range(4):
                ...
                def f_re(p):
                    ...
                    idx += n
                    self.eta = p[idx:idx + (n := self.eta.numel())].view(self.eta.shape)
                    idx += n
            ...
            O = (O - O_mean) / math.sqrt(self.step)
            ...
            L = torch.linalg.cholesky(torch.mm(O.to(torch.float64),torch.mm(torch.sqrt(torch.diag((1 / nu).to(torch.float64))),O.to(torch.float64).T)) + self.lam * torch.eye(2*self.step,dtype=torch.float64,device=device)).to(dtype1)
            parameter_k = torch.mv(torch.sqrt(torch.diag(1 / nu)), torch.mv(O.T, torch.cholesky_solve((E - torch.mv(O, self.mu * parameter.detach())).reshape(2*self.step, 1), L).reshape(2*self.step))) + self.mu * parameter.detach()
            nu = self.beta0 * nu + (parameter_k.detach() - parameter.detach()) ** 2
            parameter = parameter_k.detach()
            parameter_k *= 0.006 /(1 + max(i - 8000, 0) / 8000)
            with torch.no_grad():
                idx = 0
                ...
                eta_im_size = self.eta.numel()
                self.eta += parameter_k[idx:idx + eta_im_size].reshape(self.eta.shape)
                idx += eta_im_size
                print(idx)
            torch.cuda.empty_cache()
